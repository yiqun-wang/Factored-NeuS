...
def load_nerfactor_mask(path):
    alpha = cv.imread(path, cv.IMREAD_UNCHANGED)  # ...,4
    alpha = alpha[:, :, 3]  # h,w
    alpha = np.float32(alpha) / 255.0
    object_mask = alpha > 0.5
...
def get_camera_params(uv, pose, intrinsics):
    ...
    x_cam = uv[:, :, 0].view(batch_size, -1)  # [1,num_samples]
    y_cam = uv[:, :, 1].view(batch_size, -1)  # [1,num_samples]
    z_cam = depth.view(batch_size, -1)  # [1,num_samples]

    pixel_points_cam = lift(x_cam, y_cam, z_cam, intrinsics=intrinsics)  # image->camera [1,num_samples,4]

    # permute for batch matrix product
    pixel_points_cam = pixel_points_cam.permute(0, 2, 1)  # [1,4,num_samples]

    world_coords = torch.bmm(p, pixel_points_cam).permute(0, 2, 1)[:, :, :3]  # camera->world [1,num_samples,3]
...
def get_sphere_intersection(cam_loc, ray_directions, r = 1.0):
    ...
    cam_loc = cam_loc.unsqueeze(-1)  # [1,3,1]
